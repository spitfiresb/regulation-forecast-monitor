...
The document ACTION field and abstract distinguish final publication, proposal, delay, withdrawal, comment-period update, correction, amendment, and related notice. Dates come from structured publication fields, with the original DATES wording retained for review. For delay notices, an unambiguous explicit present-tense postponement in DATES supersedes conflicting metadata, with a visible discrepancy note and the raw metadata preserved. Conflicting or invalid new dates remain unresolved. Same-day conflicting actions, corrections, missing original rules, or unresolved identities are not forced into a simple progression. A past effective date does not prove a rule is legally in force.
...
The structured response is validated for applicable events, eligible citations, a latest-publication citation, and absence of numerical probabilities. A separate model call reviews evidence, comparative reasoning, and timing. A rejected draft may be revised once against that feedback and the same retrieved evidence, within the request time budget, then validated and reviewed again. Both attempts are archived; a second rejection still withholds the forecast. It rejects summaries of the present stage masquerading as predictions. This critique uses the same underlying model and is not independent human verification. Transient 429 and 503 provider failures can retry at most twice, honoring Retry-After when supplied and otherwise using bounded backoff, only within the remaining request budget. The request has a bounded time budget; missing configuration, unavailable sources, failed validation, or failed review withholds the prediction. There is no deterministic prediction fallback. Cached cases require current method, model, and prompt versions; unsuccessful research is retried on the next request.
