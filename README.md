...
Browse using agency and publication dropdowns. A **View some nice examples** dropdown below the filters links to selected publications through `/?example=DOCUMENT_ID`. Every click fetches current sources and runs fresh AI research and review, bypassing the one-hour assessment cache. No prerecorded forecast text is served. The disclaimer explains that these selected results are not representative of most records. Floating Home, How it Works, and purpose controls provide navigation. The purpose dialog explains the early MVP and future direction. `/how-it-works` explains the AI pipeline and links to the current system and data model.
...
Research actions, exact inspected excerpts, comparison metadata, the forecast contract, model review, prompt version, and input hash are archived in the existing case JSON. No database migration is required. The current version is `research-event-agent-v4`, with prompt `research-event-forecast-v7`. The older `npm run gemini:check` still checks the separate legacy abstract summarizer.
